...
func GenerateProofResponseFromProofV2(p vrfkey.Proof, s PreSeedDataV2) (coordinator.VRFProof, coordinator.VRFCoordinatorV2RequestCommitment, error) {
	var proof coordinator.VRFProof
	var rc coordinator.VRFCoordinatorV2RequestCommitment
	...
	return coordinator.VRFProof{
		...
		}, coordinator.VRFCoordinatorV2RequestCommitment{
			BlockNum:         s.BlockNum,
			SubId:            s.SubId,
...
func GenerateProofResponse(key vrfkey.KeyV2, s PreSeedData) (
...
func GenerateProofResponseV2(key vrfkey.KeyV2, s PreSeedDataV2) (
	coordinator.VRFProof, coordinator.VRFCoordinatorV2RequestCommitment, error) {
	seedHashMsg := append(s.PreSeed[:], s.BlockHash.Bytes()...)
	seed := utils.MustHash(string(seedHashMsg)).Big()
	proof, err := key.GenerateProof(seed)
	if err != nil {
		return coordinator.VRFProof{}, coordinator.VRFCoordinatorV2RequestCommitment{}, err
	}
